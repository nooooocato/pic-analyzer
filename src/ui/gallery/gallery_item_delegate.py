...
class GalleryItemDelegate(QStyledItemDelegate):
    """Custom delegate that respects system palette and handles rendering of gallery items."""
    ...
    def paint(self, painter: QPainter, option, index):
        ...
        if icon:
            # Add a small margin for the thumbnail
            icon_rect = option.rect.adjusted(4, 4, -4, -4)
            icon.paint(painter, icon_rect, Qt.AlignCenter)

        # Draw Checkbox in selection mode
        view = self.parent()
        # Ensure we can check for selection_mode_enabled on the view
